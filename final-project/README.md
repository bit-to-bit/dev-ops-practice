...
- `modules/`
  ...
  - `monitoring/` — Helm-установка kube-prometheus-stack (Prometheus, Grafana).
- `charts/`
...
- **charts/django-app** (Helm-чарт):
  ...
  - **Чому важливо**: Helm спрощує управління складними додатками, дозволяючи параметризацію, версіяність та повторне використання. ConfigMap та Secret ізолюють конфігурацію, HPA забезпечує авто-масштабування.
  - **Ключові компоненти**: Deployment для Django та Postgres; Service (LoadBalancer) для зовнішнього доступу; ConfigMap та Secret для безпечного зберігання конфігурації (паролі зберігаються зашифрованими); PersistentVolumeClaim (emptyDir для тестування) для даних; HorizontalPodAutoscaler для масштабування на основі CPU.
...
- **monitoring**:
  - **Призначення**: Інсталює `kube-prometheus-stack` (Prometheus, Grafana, Alertmanager) через Helm.
  - **Чому важливо**: Забезпечує централізований збір метрик та моніторинг інфраструктури кластера, дозволяючи відстежувати стан EKS та застосунків.

- **rds**:
  - **Призначення**: Універсальний модуль для створення баз даних в AWS. Підтримує розгортання як звичайних RDS (PostgreSQL, MySQL), так і кластерів Aurora. Автоматично створює DB Subnet Group, Security Group та Parameter Group.
  - **Чому важливо**: Дозволяє гнучко перемикатися між Aurora Cluster та звичайною RDS за допомогою одного прапорця `use_aurora = true/false`, мінімізуючи дублювання коду.
  - **Як змінити конфігурацію бази даних**:
    - **Зміна типу БД**: Встановіть `use_aurora = false` для звичайної RDS або `true` для Aurora Cluster.
    - **Зміна рушія (engine)**: Задайте значення для `engine` (наприклад, `"postgres"`, `"mysql"`, `"aurora-postgresql"`). Оновіть `engine_version` та `parameter_group_family`.
    - **Зміна класу інстансу**: Встановіть `instance_class` (наприклад, `"db.t3.micro"` або `"db.r6g.large"`).
    - **Висока доступність**: Встановіть `multi_az = true`.
  - **Змінні модуля**:
    - `use_aurora` (bool, default: false) - перемикач типу бази даних.
    - `engine`, `engine_version`, `parameter_group_family` (string) - конфігурація рушія.
    - `instance_class` (string) - клас інстансу.
    - `multi_az` (bool) - розгортання в декількох AZ.
    - `db_name`, `username`, `password`, `db_port` - дані для доступу.
    - `vpc_id`, `subnet_ids` - налаштування мережі.
